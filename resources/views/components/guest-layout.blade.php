...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" x-data="{ darkMode: localStorage.getItem('darkMode') === 'true' }" x-init="$watch('darkMode', val => localStorage.setItem('darkMode', val))" :class="{ 'dark': darkMode }">
    <head>
        ...
        <title>{{ $title ?? 'Secondbray - Jual Beli HP Bekas' }}</title>
    ...
    <body class="font-sans text-gray-900 antialiased dark:text-gray-100 dark:bg-gray-900">
        <div class="min-h-screen flex flex-col sm:justify-center items-center pt-6 sm:pt-0 bg-gray-100 dark:bg-gray-900">
            <div>
                <a href="/">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-2xl font-bold text-gray-900 dark:text-white">Secondbray</span>
                    </div>
                </a>
            </div>
            ...
            <!-- Dark Mode Toggle -->
            <button @click="darkMode = !darkMode" class="fixed bottom-4 right-4 p-3 bg-white dark:bg-gray-800 rounded-full shadow-lg hover:shadow-xl transition-shadow">
